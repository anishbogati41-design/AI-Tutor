...
export function QuestionManager({ fixedLessonId }: { fixedLessonId?: string }) {
  const queryClient = useQueryClient();
  const [lessonId, setLessonId] = useState(fixedLessonId ?? "");
  ...
  useEffect(() => {
    if (fixedLessonId) setLessonId(fixedLessonId);
  }, [fixedLessonId]);
        ...
        <>
          ...
          <div className="mt-8">
            ...
            <select
              id="admin-lesson"
              value={lessonId}
              disabled={Boolean(fixedLessonId)}
              onChange={(event) => {
